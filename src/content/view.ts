...
import { Position, AppearanceSettings } from "../types";
...
export class PasteView {
  ...
  constructor(
    private defaultAppearanceSettings: AppearanceSettings = {
      menuTheme: "light",
      menuPosition: "cursor",
    }
  ) {}
  ...
  displayPasteMenu(
    ...
    position: Position,
    appearance: AppearanceSettings = this.defaultAppearanceSettings
  ): void {
    // 既存のメニューを削除
  ...
  // メニュー外クリックハンドラを設定
  setClickOutsideHandler(handler: (event: MouseEvent) => void): void {
    // 既存のハンドラを削除
    if (this.clickOutsideHandler) {
      document.removeEventListener("click", this.clickOutsideHandler, true);
    }
  ...
  // メニューにクリックハンドラを設定
  setMenuClickHandler(handler: () => void): void {
    if (this.menuElement) {
  ...
  // テーマスタイルを取得
  private getThemeStyle(theme: "light" | "dark" | "system"): {
    background: string;
    color: string;
    border: string;
    boxShadow: string;
